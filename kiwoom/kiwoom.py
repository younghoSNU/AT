        ...
        if sRealType == "장시작시간":
            ...
            if sCode in self.account_stock_dict.keys() and sCode not in self.jango_dict.keys():
                ...
                if asd['매매가능수량'] > 0 and (meme_rate > 5 or meme_rate < -5):
                    order_success = self.dynamicCall(
                        ...
                        ["신규매도", self.portfolio_stock_dict[sCode]["주문용스크린번호"], self.moeuiAccount_num, 2, sCode,
                ...
                if jd['주문가능수량'] > 0 and (meme_rate > 5 or meme_rate < -5):
                    order_success = self.dynamicCall(
                        "SendOrder(QString, QString, QString, int, QString, int, int, QString, QString)",
                        ["신규매도", self.portfolio_stock_dict[sCode]["주문용스크린번호"], self.moeuiAccount_num, 2, sCode, jd['주문가능수량'],
                ...
                order_success = self.dynamicCall(
                    "SendOrder(QString, QString, QString, int, QString, int, int, QString, QString)",
                    ["신규매수", self.portfolio_stock_dict[sCode]["주문용스크린번호"], self.moeuiAccount_num, 1, sCode, quantity, e,
            ...
            for order_num in not_meme_list:
                ...
                if order_gubun == "매수" and not_quantity > 0 and e > meme_price:
                    order_success = self.dynamicCall(
                        "SendOrder(QString, QString, QString, int, QString, int, int, QString, QString)",
                        ["매수취소", self.portfolio_stock_dict[sCode]["주문용스크린번호"], self.moeuiAccount_num, 3, code, 0, 0,
